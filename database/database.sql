...
-- Started on 2014-10-01 14:42:06 CET
...
ALTER FUNCTION public.f_emachine(v_machineid integer, v_hostname character varying, v_address character varying, v_mflopspersecond double precision) OWNER TO postgres;
...
begin
 ...
 if (coalesce(d_machineid, 0) = 0) then
select nextval('machineid_seq') into d_machineid;
        insert into emachine(machineid, hostname, address, mflopspersecond, type, financial_cost) 
		values(d_machineid, v_hostname, v_address, v_mflopspersecond, v_type, v_financial_cost);
else
...
ALTER FUNCTION public.f_emachine(v_machineid integer, v_hostname character varying, v_address character varying, v_mflopspersecond double precision, v_type character varying, v_financial_cost double precision) OWNER TO postgres;

--
-- TOC entry 194 (class 1255 OID 79961)
-- Dependencies: 563 6
-- Name: f_activation(integer, integer, integer, integer, integer, character varying, character varying, text, text, timestamp with time zone, timestamp with time zone, character varying, text, character, text); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION f_activation(v_taskid integer, v_actid integer, v_machineid integer, v_processor integer, v_exitstatus integer, v_commandline character varying, v_workspace character varying, v_failure_tries integer, v_terr text, v_tout text, v_starttime timestamp with time zone, v_endtime timestamp with time zone, v_status character varying, v_extractor text, v_constrained character, v_templatedir text) RETURNS integer
    LANGUAGE plpgsql
    AS $$
...
select nextval('taskid_seq') into d_taskid;
        insert into eactivation(taskid, actid, machineid, processor, exitstatus, commandline, workspace, failure_tries, terr, tout, starttime, endtime, status, extractor, constrained, templatedir) values(d_taskid, v_actid, v_machineid, v_processor, v_exitstatus, v_commandline, v_workspace, v_failure_tries, v_terr, v_tout, v_starttime, v_endtime, v_status, v_extractor, v_constrained, v_templatedir);
else
...
ALTER FUNCTION public.f_activation(v_taskid integer, v_actid integer, v_machineid integer, v_processor integer, v_exitstatus integer, v_commandline character varying, v_workspace character varying, v_failure_tries integer, v_terr text, v_tout text, v_starttime timestamp with time zone, v_endtime timestamp with time zone, v_status character varying, v_extractor text, v_constrained character, v_templatedir text) OWNER TO postgres;
...
ALTER FUNCTION public.f_activity(v_actid integer, v_wkfid integer, v_tag character varying, v_status character varying, v_starttime timestamp with time zone, v_endtime timestamp with time zone, v_cactid integer) OWNER TO postgres;
...
ALTER FUNCTION public.f_cactivity(v_actid integer, v_wkfid integer, v_tag character varying, v_atype character varying, v_description character varying, v_activation character varying, v_extractor character varying, v_templatedir text) OWNER TO postgres;

--
-- TOC entry 197 (class 1255 OID 79964)
-- Dependencies: 563 6
-- Name: f_crelation(integer, integer, character varying, character varying, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION f_crelation(v_wkfid integer , v_rtype character varying, v_rname character varying, v_dep integer) RETURNS integer
    LANGUAGE plpgsql
    AS $$
declare d_relid integer;
begin
 select nextval('relid_seq') into d_relid;
 insert into crelation(wkfid,relid, rtype, rname, dependency) values(v_wkfid,d_relid, v_rtype, v_rname, v_dep);
 return d_relid;
end;
$$;


ALTER FUNCTION public.f_crelation(v_wkfid integer, v_rtype character varying, v_rname character varying, v_dep integer) OWNER TO postgres;
...
ALTER FUNCTION public.f_cworkflow(v_wkfid integer, v_tag character varying, v_description character varying) OWNER TO postgres;
...
ALTER FUNCTION public.f_del_workflow(v_tagexec character varying) OWNER TO postgres;
...
ALTER FUNCTION public.f_del_workflows(v_tag character varying) OWNER TO postgres;
...
ALTER FUNCTION public.f_relation(v_relid integer, v_actid integer, v_rtype character varying, v_rname character varying, v_filename character varying, v_dependency character varying) OWNER TO postgres;
...
ALTER FUNCTION public.f_workflow(v_wkfid integer, v_tag character varying, v_tagexec character varying, v_expdir character varying, v_wfdir character varying, v_maximumfailures integer, v_userinteraction character varying, v_reliability double precision, v_redundancy character varying) OWNER TO postgres;
...
ALTER TABLE public.actid_seq OWNER TO postgres;
...
ALTER TABLE public.cactid_seq OWNER TO postgres;
...
ALTER TABLE public.cactivity OWNER TO postgres;
...
ALTER TABLE public.cfield OWNER TO postgres;
...
ALTER TABLE public.coperand OWNER TO postgres;
...
ALTER TABLE public.copid_seq OWNER TO postgres;
...
ALTER TABLE public.crelation OWNER TO postgres;
...
ALTER TABLE public.cwkfid_seq OWNER TO postgres;
...
ALTER TABLE public.cworkflow OWNER TO postgres;
...
ALTER TABLE public.eactivation OWNER TO postgres;
...
ALTER TABLE public.eactivity OWNER TO postgres;
...
ALTER TABLE public.efile OWNER TO postgres;
...
ALTER TABLE public.emachine OWNER TO postgres;
...
ALTER TABLE public.eworkflow OWNER TO postgres;
...
begin
d_relid = v_relid;
 insert into cmapping(crelid,previousid, nextid) values(d_relid,v_previousid,v_nextid);
 return d_relid;
end;
$$;

ALTER FUNCTION public.f_cmapping(v_relid integer,v_previousid integer, v_nextid integer) OWNER TO postgres;


CREATE FUNCTION f_cmappingupdate(v_idatual integer, dependency integer) RETURNS integer
    LANGUAGE plpgsql
    AS $$
begin
 update cmapping set nextid = v_idatual WHERE previousid = dependency;
 return v_idatual;
end;
$$;

ALTER FUNCTION public.f_cmappingupdate(v_idatual integer, dependency integer) OWNER TO postgres;
...
ALTER TABLE public.fieldid_seq OWNER TO postgres;
...
ALTER TABLE public.fileid_seq OWNER TO postgres;
...
ALTER TABLE public.machineid_seq OWNER TO postgres;
...
ALTER TABLE public.relid_seq OWNER TO postgres;
...
ALTER TABLE public.wkfid_seq OWNER TO postgres;
...
ALTER TABLE ONLY crelation
    ADD CONSTRAINT crelation_pk PRIMARY KEY (relid,wkfid),
	ADD CONSTRAINT crelation_dependency_fk FOREIGN KEY (wkfid,dependency) REFERENCES cactivity(wkfid,actid) ON UPDATE CASCADE ON DELETE CASCADE;
...
-- 
-- Name: cmapping_crelid_fk; Type : FK CONSTRAINT; Schema: public; Owner: postgres
ALTER TABLE ONLY cmapping
    ...
    ADD CONSTRAINT cmapping_pk PRIMARY KEY (crelid);
...
REVOKE ALL ON TABLE cactivity FROM postgres;
GRANT ALL ON TABLE cactivity TO postgres;
...
REVOKE ALL ON TABLE cfield FROM postgres;
GRANT ALL ON TABLE cfield TO postgres;
...
REVOKE ALL ON TABLE coperand FROM postgres;
GRANT ALL ON TABLE coperand TO postgres;
...
REVOKE ALL ON TABLE crelation FROM postgres;
GRANT ALL ON TABLE crelation TO postgres;
...
REVOKE ALL ON TABLE cworkflow FROM postgres;
GRANT ALL ON TABLE cworkflow TO postgres;
...
REVOKE ALL ON TABLE eactivation FROM postgres;
GRANT ALL ON TABLE eactivation TO postgres;
...
REVOKE ALL ON TABLE eactivity FROM postgres;
GRANT ALL ON TABLE eactivity TO postgres;
...
REVOKE ALL ON TABLE efile FROM postgres;
GRANT ALL ON TABLE efile TO postgres;
...
REVOKE ALL ON TABLE emachine FROM postgres;
GRANT ALL ON TABLE emachine TO postgres;
...
REVOKE ALL ON TABLE eworkflow FROM postgres;
GRANT ALL ON TABLE eworkflow TO postgres;
